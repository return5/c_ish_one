...
printHelp() {
    printf "%s\n" "compile script for cish_one"
    printf "%s\n" "compiles cish_one and creates executable names 'cish_one'"
...
useGCC() {
	printf "%s\n" "using GCC"
	gcc @src_files -Wall -Wextra -O2 -Iheaders -finline-functions -Wswitch-enum -lncurses -std=gnu11 -ocish_one
}

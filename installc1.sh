...
if  ! (gcc -v >/dev/null 2>&1) || [ "$1" == "-c" ]; then
	...
	if (clang -v >/dev/null 2>&1); then
		clang @src_files -Iheaders -O2 -lncurses -std=gnu11 -ocish_one   #'src_files' is list of all .c files to include. 'flags' is a file which list all compiler flags to use
	else
	...
	if (gcc -v >/dev/null 2>&1); then
		echo "using gcc"
		gcc @src_files -Iheaders -O2 -lncurses -std=gnu11 -ocish_one 
	else
